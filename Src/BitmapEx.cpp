	...
	if (IsValid())
	{
		...
		for (long i=0; i<_height; i++)
		{
			...
			for (long j=0; j<_width; j++)
			{
				...
				long f_i = itofx(i);
				long f_j = itofx(j);
				long f_a = Mulfx(f_i, f_dy);
				long f_b = Mulfx(f_j, f_dx);
				long m = fxtoi(f_a);
				long n = fxtoi(f_b);
	...
	if (IsValid())
	{
		...
		for (long i=0; i<_height; i++)
		{
			...
			for (long j=0; j<_width; j++)
			{
				...
				long m = fxtoi(f_a);
				long n = fxtoi(f_b);
				long f_f = f_a - itofx(m);
				long f_g = f_b - itofx(n);
				dwSrcTotalOffset = m*m_iPitch + n*m_iBpp;
				...
				long f_w1 = Mulfx(f_1-f_f, f_1-f_g);
				long f_w2 = Mulfx(f_1-f_f, f_g);
				long f_w3 = Mulfx(f_f, f_1-f_g);
				long f_w4 = Mulfx(f_f, f_g);
				...
				long f_r1 = itofx(_GetRValue(pixel1));
				long f_r2 = itofx(_GetRValue(pixel2));
				long f_r3 = itofx(_GetRValue(pixel3));
				long f_r4 = itofx(_GetRValue(pixel4));
				long f_g1 = itofx(_GetGValue(pixel1));
				long f_g2 = itofx(_GetGValue(pixel2));
				long f_g3 = itofx(_GetGValue(pixel3));
				long f_g4 = itofx(_GetGValue(pixel4));
				long f_b1 = itofx(_GetBValue(pixel1));
				long f_b2 = itofx(_GetBValue(pixel2));
				long f_b3 = itofx(_GetBValue(pixel3));
				long f_b4 = itofx(_GetBValue(pixel4));
	...
	if (IsValid())
	{
		...
		for (long i=0; i<_height; i++)
		{
			...
			for (long j=0; j<_width; j++)
			{
				...
				long f_red=0, f_green=0, f_blue=0;
				for (long k=-1; k<3; k++)
				{
					long f = itofx(k)-f_f;
					long f_fm1 = f - f_1;
					...
					long f_RY = Divfx((f_a-Mulfx(f_4,f_b)+Mulfx(f_6,f_c)-Mulfx(f_4,f_d)),f_6);
					for (long l=-1; l<3; l++)
					{
						long f = itofx(l)-f_g;
						long f_fm1 = f - f_1;
						...
						if (f_fm1 > 0)
							f_d = Mulfx(f_fm1,Mulfx(f_fm1,f_fm1));
						long f_RX = Divfx((f_a-Mulfx(f_4,f_b)+Mulfx(f_6,f_c)-Mulfx(f_4,f_d)),f_6);
						long f_R = Mulfx(f_RY,f_RX);
						long _k = ((k+1)*4) + (l+1);
						long f_rs = itofx(_GetRValue(lpSrcData[dwSrcOffsets[_k]>>2]));
						long f_gs = itofx(_GetGValue(lpSrcData[dwSrcOffsets[_k]>>2]));
						long f_bs = itofx(_GetBValue(lpSrcData[dwSrcOffsets[_k]>>2]));
...
void CBitmapEx::_RotateNearestNeighbour(long degrees, _PIXEL bgColor)
{
	...
	if (IsValid())
	{
		...
		while ((_pitch & 3) != 0)
			_pitch++;
		long f_0_5 = ftofx(0.5f);
		long f_H = itofx(m_bih.biHeight/2);
		long f_W = itofx(m_bih.biWidth/2);
		long f_cos = ftofx(cos(-_angle));
		long f_sin = ftofx(sin(-_angle));
		...
		for (long i=0; i<_height; i++)
		{
			...
			for (long j=0; j<_width; j++)
			{
				...
				// Update bitmap
				long f_i = itofx(i-_height/2);
				long f_j = itofx(j-_width/2);
				long f_m = Mulfx(f_j,f_sin) + Mulfx(f_i,f_cos) + f_0_5 + f_H;
				long f_n = Mulfx(f_j,f_cos) - Mulfx(f_i,f_sin) + f_0_5 + f_W;
				long m = fxtoi(f_m);
				long n = fxtoi(f_n);
	...
	if (IsValid())
	{
		...
		for (long i=0; i<_height; i++)
		{
			...
			for (long j=0; j<_width; j++)
			{
				...
				if ((m > 0) && (m < m_bih.biHeight-1) && (n > 0) && (n < m_bih.biWidth-1))
				{
					long f_f = f_m - itofx(m);
					long f_g = f_n - itofx(n);
					dwSrcTotalOffset = m*m_iPitch + n*m_iBpp;
					DWORD dwSrcTopLeft = dwSrcTotalOffset;
					DWORD dwSrcTopRight = dwSrcTotalOffset + m_iBpp;
					if (n >= m_bih.biWidth-1)
					...
					long f_b3 = itofx(_GetBValue(pixel3));
					long f_b4 = itofx(_GetBValue(pixel4));
					long f_red = Mulfx(f_w1, f_r1) + Mulfx(f_w2, f_r2) + Mulfx(f_w3, f_r3) + Mulfx(f_w4, f_r4);
					long f_green = Mulfx(f_w1, f_g1) + Mulfx(f_w2, f_g2) + Mulfx(f_w3, f_g3) + Mulfx(f_w4, f_g4);
					long f_blue = Mulfx(f_w1, f_b1) + Mulfx(f_w2, f_b2) + Mulfx(f_w3, f_b3) + Mulfx(f_w4, f_b4);
...
void CBitmapEx::_ShearVerticalNearestNeighbour(long degrees, _PIXEL bgColor)
{
	...
	if (IsValid())
	{
		...
		long f_alpha = ftofx(_alpha);
		long f_offset = ftofx(_offset);
	...
	if (IsValid())
	{
		...
		for (long i=0; i<_height; i++)
		{
			...
			for (long j=0; j<_width; j++)
			{
				...
				if ((m >= 0) && (m <= m_bih.biHeight-1) && (n >= 0) && (n <= m_bih.biWidth-1))
				{
					long f_f = f_y - itofx(m);
					long f_g = f_x - itofx(n);
					dwSrcTotalOffset = m*m_iPitch + n*m_iBpp;
					DWORD dwSrcTopLeft = dwSrcTotalOffset;
					DWORD dwSrcTopRight = dwSrcTotalOffset + m_iBpp;
					if (n >= m_bih.biWidth-1)
...
void CBitmapEx::Grayscale()
{
	...
	if (IsValid())
	{
		...
		long f_w1 = ftofx(0.299f);
		long f_w2 = ftofx(0.587f);
		long f_w3 = ftofx(0.114f);
...
/*		// Grayscale bitmap
		...
		for (long i=0; i<m_bih.biHeight; i++)
		{
			...
			for (long j=0; j<m_bih.biWidth; j++)
			{
				...
				// Update bitmap
				long f_red = itofx(_GetRValue(lpDstData[dwTotalOffset>>2]));
				long f_green = itofx(_GetGValue(lpDstData[dwTotalOffset>>2]));
				long f_blue = itofx(_GetBValue(lpDstData[dwTotalOffset>>2]));
				long f_value = Mulfx(f_w1,f_red) + Mulfx(f_w2,f_green) + Mulfx(f_w3,f_blue);
...
void CBitmapEx::Sepia(long depth)
{
	...
	if (IsValid())
	{
		...
		long f_depth = itofx(_depth);
		long f_depth2 = Mulfx(itofx(2),f_depth);
...
void CBitmapEx::Emboss()
{
	...
	if (IsValid())
	{
		...
		long f_w[9] = {itofx(-1), itofx(-1), itofx(0),
		...
		long f_w2 = ftofx(0.587f);
		long f_w3 = ftofx(0.114f);
		long f_128 = itofx(128);
		long f_255 = itofx(255);
		...
		for (long i=0; i<m_bih.biHeight; i++)
		{
			...
			for (long j=0; j<m_bih.biWidth; j++)
			{
				...
				for (long k=0; k<9; k++)
				{
					long f_r = itofx(_GetRValue(lpSrcData[dwSrcOffsets[k]>>2]));
					long f_g = itofx(_GetGValue(lpSrcData[dwSrcOffsets[k]>>2]));
					long f_b = itofx(_GetBValue(lpSrcData[dwSrcOffsets[k]>>2]));
...
void CBitmapEx::Pixelize(long size)
{
	...
	if (IsValid())
	{
		...
		long f_size = Divfx(itofx(1),itofx(_size*_size));
...
void CBitmapEx::Contrast(long contrast)
{
	...
	if (IsValid())
	{
		...
		long f_contrast = ftofx(1.0f/_contrast);
		long f_128 = itofx(128);
...
void CBitmapEx::Colorize(_PIXEL color)
{
	...
	if (IsValid())
	{
		...
		long f_sred = itofx(_GetRValue(color));
		long f_sgreen = itofx(_GetGValue(color));
		long f_sblue = itofx(_GetBValue(color));
		_PIXEL _sColor = _RGB2HSV(color);
		long f_value = Divfx(itofx(_GetBValue(_sColor)), itofx(255));
		...
		for (long i=0; i<m_bih.biHeight; i++)
		{
			...
			for (long j=0; j<m_bih.biWidth; j++)
			{
				...
				// Update bitmap
				long f_dred = itofx(_GetRValue(lpDstData[dwTotalOffset>>2]));
				long f_dgreen = itofx(_GetGValue(lpDstData[dwTotalOffset>>2]));
				long f_dblue = itofx(_GetBValue(lpDstData[dwTotalOffset>>2]));
	...
	if (IsValid())
	{
		...
		for (long i=0; i<m_bih.biHeight; i++)
		{
			...
			for (long j=0; j<m_bih.biWidth; j++)
			{
				...
				// Update bitmap
				DWORD dwSrcOffset = dwTotalOffset;
				long f_redMin=f_255, f_redMax=0;
				long f_greenMin=f_255, f_greenMax=0;
				long f_blueMin=f_255, f_blueMax=0;
				for (long k=0; k<3; k++)
				{
					long m = i + k;
					if (m >= m_bih.biHeight-1)
					...
					for (long l=0; l<3; l++)
					{
						...
						if (n >= m_bih.biWidth-1)
							n = m_bih.biWidth - 1;
						dwSrcOffset = m*m_iPitch + n*m_iBpp;
						long f_red = itofx(_GetRValue(lpSrcData[dwSrcOffset>>2]));
						long f_green = itofx(_GetGValue(lpSrcData[dwSrcOffset>>2]));
						long f_blue = itofx(_GetBValue(lpSrcData[dwSrcOffset>>2]));
						if (bMinimum)
						{
							if (f_red < f_redMin)
								f_redMin = f_red;
...
void CBitmapEx::GlowingEdges(long blur, long threshold, long scale)
{
	...
	if (IsValid())
	{
		...
		long f_treshold = ftofx(_threshold);
		long f_scale = ftofx(_scale);
		long f_255 = itofx(255);
		...
		for (long i=0; i<m_bih.biHeight; i++)
		{
			...
			for (long j=0; j<m_bih.biWidth; j++)
			{
				...
				// Update bitmap
				long f_red = Divfx(itofx(_GetRValue(lpSrcData[dwTotalOffset>>2])),f_255);
				long f_green = Divfx(itofx(_GetGValue(lpSrcData[dwTotalOffset>>2])),f_255);
				long f_blue = Divfx(itofx(_GetBValue(lpSrcData[dwTotalOffset>>2])),f_255);
...
void CBitmapEx::Solarize(long threshold)
{
	...
	if (IsValid())
	{
		...
		long f_treshold = itofx(_threshold);
		long f_255 = itofx(255);
		...
		for (long i=0; i<m_bih.biHeight; i++)
		{
			...
			for (long j=0; j<m_bih.biWidth; j++)
			{
				...
				BYTE green = _GetGValue(lpDstData[dwTotalOffset>>2]);
				BYTE blue = _GetBValue(lpDstData[dwTotalOffset>>2]);
				long f_red = itofx(red);
				long f_green = itofx(green);
				long f_blue = itofx(blue);
				if (f_red > 0)
...
void CBitmapEx::Draw(long dstX, long dstY, long width, long height, CBitmapEx& bitmapEx, long srcX, long srcY, long alpha)
{
	...
	if (IsValid() && bitmapEx.IsValid())
	{
		...
		long f_alpha = ftofx(_alphaPercent);
		long f_1 = itofx(1);
		long f_1malpha = f_1 - f_alpha;
		long _width = min(_dstEndX-_dstStartX, _srcEndX-_srcStartX);
		long _height = min(_dstEndY-_dstStartY, _srcEndY-_srcStartY);
		if ((_width == 0) || (_height == 0))
			return;
...
/*		// Draw bitmap
		...
		for (long i=0; i<=_height; i++)
		{
			...
			for (long j=0; j<=_width; j++)
			{
				...
				long f_sred = itofx(_GetRValue(pixel1));
				long f_sgreen = itofx(_GetGValue(pixel1));
				long f_sblue = itofx(_GetBValue(pixel1));
				long f_dred = itofx(_GetRValue(pixel2));
				long f_dgreen = itofx(_GetGValue(pixel2));
				long f_dblue = itofx(_GetBValue(pixel2));
						...
						for (long j=_dstStartX; j<=_dstEndX; j++)
						{
							...
							if (det >= 0.0f)
							{
								...
								if ((mu >= 0.0f) && (mu < 1.0f) && (delta >= 0.0f) && (delta < 1.0f))
								{
									...
									long f_f = ftofx(delta*_height) - itofx(m);
									long f_g = ftofx(mu*_width) - itofx(n);
									long f_1 = itofx(1);
						...
						for (long j=_dstStartX; j<=_dstEndX; j++)
						{
							...
							if (det >= 0.0f)
							{
								...
								if ((mu >= 0.0f) && (mu < 1.0f) && (delta >= 0.0f) && (delta < 1.0f))
								{
									...
									long f_f = ftofx(delta*_srcHeight+_srcStartY) - itofx(m);
									long f_g = ftofx(mu*_srcWidth+_srcStartX) - itofx(n);
									long f_1 = itofx(1);
									DWORD dwSrcTotalOffset = (_height-m-1)*_pitch + n*_bpp;
									DWORD dwSrcTopLeft = dwSrcTotalOffset;
									DWORD dwSrcTopRight = dwSrcTotalOffset + _bpp;
...
/*		// Draw bitmap
		...
		long f_f, f_g, f_w1, f_w2, f_w3, f_w4;
		__asm {
			mov edi, lpDstData
			add edi, dwDstVerticalOffset
			add edi, dwDstHorizontalStartOffset
...
/*		// Draw bitmap
		...
		for (long i=0; i<_dstHeight; i++)
		{
			...
			for (long j=0; j<_dstWidth; j++)
			{
				...
				long f_b3 = itofx(_GetBValue(pixel3));
				long f_b4 = itofx(_GetBValue(pixel4));
				long f_sred = (Mulfx(f_w1, f_r1) + Mulfx(f_w2, f_r2) + Mulfx(f_w3, f_r3) + Mulfx(f_w4, f_r4));
				long f_sgreen = (Mulfx(f_w1, f_g1) + Mulfx(f_w2, f_g2) + Mulfx(f_w3, f_g3) + Mulfx(f_w4, f_g4));
				long f_sblue = (Mulfx(f_w1, f_b1) + Mulfx(f_w2, f_b2) + Mulfx(f_w3, f_b3) + Mulfx(f_w4, f_b4));
				long f_dred = itofx(_GetRValue(lpDstData[dwDstTotalOffset>>2]));
				long f_dgreen = itofx(_GetGValue(lpDstData[dwDstTotalOffset>>2]));
				long f_dblue = itofx(_GetBValue(lpDstData[dwDstTotalOffset>>2]));
				BYTE red = (BYTE)fxtoi(Mulfx(f_sred, f_alpha) + Mulfx(f_dred, f_1-f_alpha));
				BYTE green = (BYTE)fxtoi(Mulfx(f_sgreen, f_alpha) + Mulfx(f_dgreen, f_1-f_alpha));
				BYTE blue = (BYTE)fxtoi(Mulfx(f_sblue, f_alpha) + Mulfx(f_dblue, f_1-f_alpha));
				lpDstData[dwDstTotalOffset>>2] = _RGB(red, green, blue);
		...
		DWORD m, n, dwSrcVerticalOffset, dwSrcHorizontalOffset;
		DWORD dwSrcTopLeft, dwSrcTopRight, dwSrcBottomLeft, dwSrcBottomRight;
		long f_f, f_g, f_w1, f_w2, f_w3, f_w4, f_srcPixel;
		__asm {
			mov edi, lpDstData
			add edi, dwDstVerticalOffset
			add edi, dwDstHorizontalStartOffset
	...
	if (IsValid() && bitmapEx.IsValid())
	{
		...
		for (long i=0; i<_dstHeight; i++)
		{
			...
			for (long j=0; j<_dstWidth; j++)
			{
				...
				long f_sred = itofx(max(0, min(255, fxtoi(Mulfx(f_red,f_gama)))));
				long f_sgreen = itofx(max(0, min(255, fxtoi(Mulfx(f_green,f_gama)))));
				long f_sblue = itofx(max(0, min(255, fxtoi(Mulfx(f_blue,f_gama)))));
				long f_dred = itofx(_GetRValue(lpDstData[dwDstTotalOffset>>2]));
				long f_dgreen = itofx(_GetGValue(lpDstData[dwDstTotalOffset>>2]));
				long f_dblue = itofx(_GetBValue(lpDstData[dwDstTotalOffset>>2]));
				BYTE red = (BYTE)fxtoi(Mulfx(f_sred, f_alpha) + Mulfx(f_dred, f_1-f_alpha));
	...
	if (IsValid() && bitmapEx.IsValid())
	{
		...
		for (long i=0; i<=_height; i++)
		{
			...
			for (long j=0; j<=_width; j++)
			{
				...
				_PIXEL pixel1 = lpSrcData[dwSrcTotalOffset>>2];
				_PIXEL pixel2 = lpDstData[dwDstTotalOffset>>2];
				long f_alphaSrc = f_1 - ftofx((float)_GetRValue(pixel1)/255.0f);
				long f_alphaDst = f_1 - Mulfx(f_alphaSrc, f_alpha);
				long f_sred = itofx(_GetRValue(alphaColor));
				long f_sgreen = itofx(_GetGValue(alphaColor));
				long f_sblue = itofx(_GetBValue(alphaColor));
				long f_dred = itofx(_GetRValue(pixel2));
				long f_dgreen = itofx(_GetGValue(pixel2));
				long f_dblue = itofx(_GetBValue(pixel2));
				...
				if (lpszText[k] != ' ')
				{
					...
					for (long i=0; i<cy; i++)
					{
						...
						for (long j=0; j<cx; j++)
						{
							...
							long f_value = ftofx(lpFontBitmap[index]);
							long f_alpha = Mulfx(Divfx(f_value,ftofx(65.0f)),Divfx(itofx(textAlpha),ftofx(100.0f)));
							long f_1malpha = itofx(1) - f_alpha;
							long f_sred = itofx(_GetRValue(textColor));
							long f_sgreen = itofx(_GetGValue(textColor));
							long f_sblue = itofx(_GetBValue(textColor));
							long f_dred = itofx(_GetRValue(pixel));
							long f_dgreen = itofx(_GetGValue(pixel));
							long f_dblue = itofx(_GetBValue(pixel));
...
void CBitmapEx::ReplaceColor(long x, long y, _PIXEL newColor, long alpha, long error, BOOL bImage)
{
	...
	if ((IsValid()) && (m_ColorMode == CM_RGB))
	{
		...
		long f_1 = itofx(1);
		long f_alpha = ftofx(_alpha);
		long f_dred = itofx(_GetRValue(newColor));
		long f_dgreen = itofx(_GetGValue(newColor));
		long f_dblue = itofx(_GetBValue(newColor));
		...
		if (bImage)
		{
			...
			for (long i=0; i<m_bih.biHeight; i++)
			{
				...
				for (long j=0; j<m_bih.biWidth; j++)
				{
					...
					if (_diff < _error)
					{
						long f_sred = itofx(_GetRValue(lpDstData[dwTotalOffset>>2]));
						long f_sgreen = itofx(_GetGValue(lpDstData[dwTotalOffset>>2]));
						long f_sblue = itofx(_GetBValue(lpDstData[dwTotalOffset>>2]));
	...
	if (IsValid() && (m_lpFire != NULL))
	{
		...
		for (i=0; i<m_bih.biHeight; i++)
		{
			...
			for (j=0; j<m_bih.biWidth; j++)
			{
				...
				// Update bitmap
				long f_red = itofx(m_lpFirePalette[m_lpFire[dwOffset]].rgbRed);
				long f_green = itofx(m_lpFirePalette[m_lpFire[dwOffset]].rgbGreen);
				long f_blue = itofx(m_lpFirePalette[m_lpFire[dwOffset]].rgbBlue);
				long f_value = Mulfx(f_w1,f_red) + Mulfx(f_w2,f_green) + Mulfx(f_w3,f_blue);
				long f_alpha = Divfx(f_value,ftofx(255.0f));
				long f_1malpha = itofx(1) - f_alpha;
				long f_sred = itofx(_GetRValue(lpSrcData[dwTotalOffset>>2]));
				long f_sgreen = itofx(_GetGValue(lpSrcData[dwTotalOffset>>2]));
				long f_sblue = itofx(_GetBValue(lpSrcData[dwTotalOffset>>2]));
				long f_dred = Mulfx(f_1malpha,f_sred) + Mulfx(f_alpha,f_red);
				long f_dgreen = Mulfx(f_1malpha,f_sgreen) + Mulfx(f_alpha,f_green);
				long f_dblue = Mulfx(f_1malpha,f_sblue) + Mulfx(f_alpha,f_blue);
				lpDstData[dwTotalOffset>>2] = _RGB(fxtoi(f_dred), fxtoi(f_dgreen), fxtoi(f_dblue));
				dwOffset++;
	...
	if (IsValid())
	{
		...
		for (long i=0; i<m_bih.biHeight; i++)
		{
			...
			for (long j=0; j<m_bih.biWidth; j++)
			{
				...
				long f_value = ftofx(value);
				long f_alpha = Divfx(f_value,ftofx(255.0f));
				long f_1malpha = itofx(1) - f_alpha;
				long f_sred = itofx(_GetRValue(lpSrcData[dwTotalOffset>>2]));
				long f_sgreen = itofx(_GetGValue(lpSrcData[dwTotalOffset>>2]));
				long f_sblue = itofx(_GetBValue(lpSrcData[dwTotalOffset>>2]));
				long f_dred = Mulfx(f_1malpha,f_sred) + Mulfx(f_alpha,f_value);
				long f_dgreen = Mulfx(f_1malpha,f_sgreen) + Mulfx(f_alpha,f_value);
				long f_dblue = Mulfx(f_1malpha,f_sblue) + Mulfx(f_alpha,f_value);
				lpDstData[dwTotalOffset>>2] = _RGB(fxtoi(f_dred), fxtoi(f_dgreen), fxtoi(f_dblue));
